...
class DeuteronNQS(nn.Module):
    ...
    def forward(self, x):
        # [-L/2, L/2]. Box.
        dx = ((x - self.potential_center + self.L / 2.0) % self.L) - (self.L / 2.0)

        # From 3D to 6D. x --> 2*pi*x/L.   x --> (sin(2*pi*x/L), cos(2*pi*x/L)).
        arg = (2.0 * math.pi / self.L) * dx
        features = torch.cat([torch.cos(arg), torch.sin(arg)], dim=1)  # (N_walkers, 6).

        # r^{2}= Xs^2+Ys^2+Zs^2. Xs=(L/pi)*sin(pi*dx/L).
        x_sin = (self.L / math.pi) * torch.sin(math.pi * dx / self.L)
        r2_sin = torch.sum(x_sin**2, dim=1, keepdim=True)
        r_sin_aux = torch.sqrt(r2_sin + 0.5**2) - 0.5

        # Amplitude network.
        amplitude1 = self.actfun(self.amp_lc1(features))
        amplitude2 = self.actfun(self.amp_lc2(amplitude1))
        amplitude = self.amp_lc3(amplitude2) - self.alpha * r_sin_aux

        # Phase network.
        phase1 = self.actfun(self.phase_lc1(features))
        phase2 = self.actfun(self.phase_lc2(phase1))
        phase = self.phase_lc3(phase2) + self.gamma * r2_sin
    ...
    def thermalization(self, walkers: torch.Tensor, step_size: float) -> torch.Tensor:
        ...
        for _ in range(500):
            ...
            # PBC on the walkers too.
            walkers = ((walkers + self.L / 2.0) % self.L) - (self.L / 2.0)
        return walkers
...
def local_energy(net, walkers, config):
    ...
    # r2 considering PBC.
    dx = ((walkers - center_potential + net.L / 2.0) % net.L) - (net.L / 2.0)
    r2=torch.sum(dx**2, dim=1, keepdim=True)
