...
import math
from ground_state_PBC import train_ground_state, DeuteronNQS, local_energy
import os
from time_evolution_PBC import run_time_evolution
...
def main():
    ...
    # Mesh for grid calculations, considering the box.    
    box_L = config['L']
    Nx = config['Nx']
    dx_mesh = box_L / Nx
    coords = torch.linspace(-box_L / 2.0, (box_L / 2.0) - dx_mesh, Nx)
    ...
    base_filename = (f"PBC_MassFactor_{config['mass_factor']}_"
    ...
    run_folder = os.path.join(script_dir, "Plots_PBC", base_filename)
    os.makedirs(run_folder, exist_ok=True)

    # Load exact numerical results.
    exact_data_gs = np.load(os.path.join(
        exact_folder, "exact_deuteron_target_C_10_142_Mass_Factor_1.npz"))
    ...
    exact_data_te = np.load(os.path.join(
        exact_folder, "exact_time_evolution_0.00075_Mass_Factor_1.npz"))
    ...
    net = DeuteronNQS(W1_amp, B1_amp, W2_amp, B2_amp, W3_amp,
                      ...
                      config['x0'], config['y0'], config['z0'], L=config['L']).to(device)
    ...
    def pic_callback(i, psi_mesh, psi_norm_mesh, loss_accum, U_accum, K_accum):
        ...
        def update_plot(ax, x_data, y_nqs, y_tar, title):
            ...
            if ax.lines:
                ...
                ax.plot(x_data, y_nqs, label='$\psi_{NQS}$', color='b')
                ax.plot(x_data, y_tar, '--',
                        label='$\psi_{Target}$', color='r')
                ax.legend(loc='upper right', fontsize='x-small')

        update_plot(ax1, coords_plot, slice_x_nqs,
                    target_slice, "Normalized $\psi(x,y0,z0)$")
        update_plot(ax2, coords_plot, slice_y_nqs,
                    target_slice, "Normalized $\psi(x0,y,z0)$")
        update_plot(ax3, coords_plot, slice_z_nqs,
                    target_slice, "Normalized $\psi(x0,y0,z)$")
        ...
        ax4.cla()
        ax1.set_xlim(-15.4, 15.4)
        ax2.set_xlim(-15.4, 15.4)
        ax3.set_xlim(-15.4, 15.4)
        ax4.set_xlim(0, i+1)
        ax4.set_ylim(-2.35, 0)
        ...
        ax4.plot(x_epochs, y_U, label='$U$',
                 color='orange', linewidth=0.8, alpha=0.8)
        ax4.plot(x_epochs, y_K, label='$K$',
                 color='green', linewidth=0.8, alpha=0.8)
        ax4.plot(x_epochs, np.ones_like(x_epochs)*(E_exact_target), '--',
                 label=rf'$E_{{exact}} \approx {E_exact_target:.2f}$ MeV', color='k', alpha=0.6)
    ...
    r_nqs = torch.linspace(0.01, 10, 500, device=device)
    ...
    plt.plot(r_nqs_np, psi_nqs_normalized, 'b',
             label='NQS $\psi(r)$', linewidth=1.5)
    plt.plot(r_exact_safe, psi_exact_safe, 'r--',
             label='Numerical $\psi(r)$', alpha=0.8, linewidth=1.5)
    plt.xlim(0, 10)
    ...
    plt.legend()
    plt.savefig(os.path.join(run_folder, f"2_walkers_initial.png"), dpi=300)
    ...
    plt.figure(figsize=(8, 5), dpi=300)
    plt.hist(x_coords, bins=80, density=True,
            alpha=0.6, color='C0', label='Walkers X')
    plt.hist(y_coords, bins=80, density=True,
            alpha=0.6, color='C1', label='Walkers Y')
    plt.hist(z_coords, bins=80, density=True,
            alpha=0.6, color='C2', label='Walkers Z')
    ...
    plt.grid(alpha=0.2)
    plt.savefig(os.path.join(
        run_folder, f"3_walkers_final.png"), dpi=300)
    ...
    plt.axhline(y=r_exact_target, color='r', linestyle='--',
                label=rf'Numerical $\langle r \rangle \approx$ {r_exact_target:.3f} fm')
    plt.xlim(0, aux)
    plt.ylim(0.9*r_exact_target, 1.15*r_exact_target)
    plt.xlabel("Epoch")
    plt.ylabel("$\langle r \\rangle$ (fm)")
    plt.legend(loc='lower right')
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(
        run_folder, f"6_r_zoomed.png"), dpi=300)
    ...
    plt.axhline(y=r2_exact_target, color='r', linestyle='--',
                label=rf'Numerical $\frac{{\sqrt{{\langle r^2 \rangle}}}}{{2}} \approx$ {r2_exact_target:.3f} fm')
    plt.xlim(0, aux)
    plt.ylim(0.9*r2_exact_target, 1.15*r2_exact_target)
    plt.xlabel("Epoch")
    plt.ylabel(r"$\frac{\sqrt{\langle r^2 \rangle}}{2}$ (fm)")
    plt.legend(loc='lower right')
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(
        run_folder, f"7_r_sq_zoomed.png"), dpi=300)
    ...
    plt.plot(x_epochs, y_loss, label='$E$', color='b', linewidth=1.2)
    plt.plot(x_epochs, y_U, label='$U$', color='orange', linewidth=1)
    plt.plot(x_epochs, y_K, label='$K$', color='green', linewidth=1)
    ...
    plt.legend()
    plt.savefig(os.path.join(
        run_folder, f"8_energy_zoomed_out.png"), dpi=300)

    # Energy for prints.
    EL, UL, KL, logpsi_walkers = local_energy(net, walkers, config)
    with torch.no_grad():
        E_var = torch.var(EL).real
        E_std = torch.std(EL).real
        E0_err = E_std / math.sqrt(config['n_walkers'])
        E0 = loss_accum[-1]

    print('Acceptance =', acceptance)
    print(f"Energy: {E0:.4f} ± {E0_err:.4f}")
    print(f"Potential Energy: {y_U[-1]:.4f}")
    print(f"Kinetic Energy: {y_K[-1]:.4f}")
    ...
    plt.xlabel(r'Time $t$ ($\mathrm{fm}/c$)')
    plt.legend(loc="best")
    plt.grid(alpha=0.3)
    plt.ylim(2.81, 2.962)
    ...
    plt.savefig(os.path.join(run_folder, "13_energy.png"), bbox_inches='tight', dpi=300)
    plt.savefig(os.path.join(run_folder, "energy.pdf"), bbox_inches='tight')
    plt.close()

    # <r> again.
    fig_r, ax_r = plt.subplots(figsize=(6.75, 4.0), dpi=300)

    ax_r.plot(
        time_history, r_mean_walkers_history,
        color='green',
        linewidth=1.5,
        label=r"NQS $\langle r \rangle$ (Walkers)"
    )

    ax_r.fill_between(
        time_history,
        r_mean_walkers_history - r_mean_walkers_err_history,
        r_mean_walkers_history + r_mean_walkers_err_history,
        color='green',
        alpha=0.3
    )
    ...
    ax_r.plot(
        time_history, r_mean_history,
        color='red',
        linewidth=1.5,
        label=r"NQS $\langle r \rangle$ (Mesh Integral)"
    )
    ...
    ax_r.set_xlim(0, time_history[-1])
    ax_r.set_ylim(2.81, 2.962)
    ax_r.set_xlabel(r'Time $t$ ($\mathrm{fm}/c$)')
    ax_r.set_ylabel(r'$\langle r \rangle$ (fm)')
    ax_r.legend(loc='best', fontsize=8)
    ax_r.grid(alpha=0.3)

    fig_r.tight_layout()

    fig_r.savefig(os.path.join(run_folder, "Radius_evolution.png"), dpi=300, bbox_inches='tight')
    fig_r.savefig(os.path.join(run_folder, "Radius_Evolution_Comparison.pdf"), bbox_inches='tight')
    plt.close(fig_r)
