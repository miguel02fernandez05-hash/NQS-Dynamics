...
from ground_state import local_energy
...
def step_time_rk2(net, walkers, dt, config, step_size, steps_per_iteration=100):
...
def step_time_rk4(net, walkers, dt, config, step_size, steps_per_iteration=100):
...
def run_time_evolution(net, walkers, config, exact_data_te, X_train):
    ...
    walkers, _ = net.metropolis_step(walkers,config['step_sizeRK'],N_acc=0,steps_per_iteration=100)
    ...
    for t in tqdm(range(1,time_steps+1), desc="Time Evolution"):
        ...
        walkers, EL = step_time_rk2(
            net, walkers, dt, config, step_size, steps_per_iteration=100)
